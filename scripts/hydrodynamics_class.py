...
class Hydro:
    ...
    def get_number_of_star_particles(self):
        return len(self.star_particles)
    
    def write_set_to_file(self, index):
        filename = "hydro_molecular_cloud_collapse_i{0:04}.txt".format(index)
        write_set_to_file(self.gas_particles, filename, "txt")
        if len(self.star_particles):
            write_set_to_file(self.star_particles, filename, "txt", append_to_file=True)  # , attribute_names=self.star_attributes)
